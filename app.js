...
mongoose.connect('mongodb+srv://admin-manish:test-123@cluster0.yebglx5.mongodb.net/blogdb');
...
app.get("/", (req, res) => {
  ...
  blog.find({}, (err, posts) => {
    ...
    res.render("home", { homeContent: homeStartingContent, postDec: posts });
  })
})
...
app.get("/posts/:postID", (req, res) => {
  ...
  // posts.forEach((ele)=>{
  // const postTitle = _.lowerCase(ele.title);
  // if(postTitle === reqTitle){
    // }
    // })
    blog.findOne({ _id : req.params.postID }, (err, ele) => {
      if(!err){
